...
class SchemaGenerator:
    ...
    BOOLEAN_MATCHER_DEFAULT = ['true', 'false']

    # Detect a TIME field of the form [H]H:[M]M:[S]S[.DDDDDD]
    # TIME_MATCHER = re.compile(r'^\d{1,2}:\d{1,2}:\d{1,2}(\.\d{1,6})?$')
    EVOLVE_TIME_MATCHER = re.compile(r'^\d{1,2}:\d{1,2}:\d{1,2}(\.\d{1,3})\+\d{1,4}?$')

    # YYYY-[M]M-[D]D[( |T)[H]H:[M]M:[S]S[.DDDDDD]][time zone]
    EVOLVE_DATETIME_MATCHER = re.compile(r'^\d{4}-\d{1,2}-\d{1,2}T\d{1,2}:\d{1,2}:\d{1,2}(\.\d{1,3})[+-]\d{4}?$')
    ...
    def get_schema_entry(self, key, value, base_path=None):
        ...
        value_mode, value_type = self.infer_bigquery_type(key, value)
    ...
    def infer_bigquery_type(self, node_key, node_value):
        """Determines the BigQuery (mode, type) tuple of the right hand side of
        the node value.
        """
        node_type = self.infer_value_type(node_key, node_value)
    ...
    def infer_value_type(self, key, value):
        ...
        * Integers which overflow signed 64-bit are considered to be floats, for
        ...
        if isinstance(value, str):
            ...
            if self.DATE_MATCHER.match(value):
                return 'DATE'
            elif self.EVOLVE_TIME_MATCHER.match(value):
                return 'EVOLVE_TIME'
            # elif self.TIMESTAMP_MATCHER.match(value):
            #     return 'TIMESTAMP'
            elif self.EVOLVE_DATETIME_MATCHER.match(value):
                return 'EVOLVE_DATETIME'
...
def get_type_transform(column_name, schema_type):
    if schema_type == 'EVOLVE_TIME':
        return f"SUBSTR({column_name}, 0 ,12)"
    # elif schema_type == 'EVOLVE_DATETIME':
    #     return f"SUBSTR({column_name}, 0 ,23)"
    else:
        return column_name

def get_type_definition(schema_type):
    if schema_type == 'FLOAT':
        target_type = 'NUMERIC'
    elif schema_type == 'EVOLVE_TIME':
        target_type = 'TIME'
    elif schema_type == 'EVOLVE_DATETIME':
...
def create_tbl_vw_newschema(src_project_id, src_dataset_id, src_table_id, trg_project_id, trg_dataset_id, trg_view_id):
    client = bigquery.Client()
    query_selectall = f"""
        SELECT *
        FROM `{src_project_id}.{src_dataset_id}.{src_table_id}`
    """
    query_job = client.query(query_selectall)
    result = query_job.result()
    schema = result.schema

    # check if columns contains non-STRING type
    all_str = True
    for j in schema:
        if j.field_type != 'STRING':
            all_str = False
            break
    ...
    if all_str:
        ...
        table_schema = get_schema(result_rows)
        new_dict_table_schema = {item['name']: item for item in table_schema}
        headers_in_order = []
        schema_transform_list = []
        for a in result.schema:
            headers_in_order.append(a.name)

        # make sure the schema generator comes with the same header order with the source table
        for i in headers_in_order:
            schema_detail = new_dict_table_schema[i]

            schema_transform_list.append("SAFE_CAST({} AS {}) AS {}".format(
                                                                    get_type_transform(i, schema_detail['type']),
                                                                    get_type_definition(schema_detail['type']),
                                                                    i)
            )
        schema_transform = ',\n'.join(schema_transform_list)
        schema_columns = ',\n'.join(headers_in_order)

        query_createview = f"""
        CREATE OR REPLACE VIEW `{trg_project_id}.{trg_dataset_id}.{trg_view_id}`
        (
            {schema_columns}
        )
        AS
        ...
        print(query_createview)

        query_job_vw = client.query(query_createview)
        return 'Transformation View have been Created'
    else:
...
def auto_schema_detect(request):
    ...
    for call in calls:
        src_project_id = call[0]
        src_dataset_id = call[1]
        src_table_id = call[2]
        trg_project_id = call[3]
        trg_dataset_id = call[4]
        trg_view_id = call[5]
        break

    print(
        'input project id: {}. input dataset id: {}. input table id : {}. target project id: {}. target dataset id: {}. target view id: {}',
        src_project_id, src_dataset_id, src_table_id, trg_project_id, trg_dataset_id, trg_view_id)
    message = create_tbl_vw_newschema(src_project_id,
                                      src_dataset_id,
                                      src_table_id,
                                      trg_project_id,
                                      trg_dataset_id,
